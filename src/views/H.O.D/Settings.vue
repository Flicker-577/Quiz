        ...
        <div class="card">
          ...
          <div class="form-container">
            <div v-if="loading" class="form-grid single-col">
              <div v-for="i in 3" :key="i" class="form-group skeleton-group">
                 <AppSkeleton type="text" width="120px" height="16px" class="mb-2"/>
                 <AppSkeleton type="text" width="100%" height="42px" borderRadius="8px" />
              </div>
            </div>
    ...
    >
      ...
      <template #footer>
        <div class="modal-actions centered">
          ...
          >
            <span v-if="isProcessing" class="btn-loading-content">
              <AppSpinner size="sm" color="light" /> Processing...
            </span>
            <span v-else>{{ confirm.btnText }}</span>
          </AppButton>
        </div>
      </template>
    </AppModal>
...
export default {
  ...
  setup() {
    ...
    const fetchProfile = async () => {
      loading.value = true
      // UPDATED: Added delay for smoother skeleton transition
...
<style scoped>
/* UPDATED: Increased max-width to match other admin pages */
.settings-page { max-width: 1200px; margin: 0 auto; padding: var(--spacing-md); }
...
/* UPDATED: Container to keep forms from getting too wide on large screens */
.form-container { max-width: 800px; }

.form-grid.single-col { display: flex; flex-direction: column; gap: var(--spacing-md); width: 100%; }
...
.modal-actions.centered { justify-content: center; gap: 12px; display: flex; margin-top: 20px; }
